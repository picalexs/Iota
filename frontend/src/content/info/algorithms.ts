...
export const ALGORITHM_INFO: Record<RunAlgorithm, InfoEntry> = {
  ...
  sqd: {
    ...
    sections: [
      ...
      {
        heading: "Workflow in practice",
        body: "A run samples bitstrings, validates electron-number sectors, optionally repairs invalid strings using occupancy estimates, splits the recovered determinants into selected-CI batches, diagonalizes the projected Hamiltonian in those batches, and feeds the resulting occupancies back into the next recovery round. Quantum Studio follows the qiskit-addon-sqd workflow closely and reports the best observed recovery iteration as the top-level SQD energy rather than blindly trusting the last iteration.",
      },
      {
        heading: "Controls that change behavior",
        body: "The sample budget is controlled primarily by `samples_per_batch` and `num_batches`. The recovery loop is controlled by `max_iterations`, `energy_tol`, `occupancies_tol`, and `min_selected_configurations`. The size and cost of the classical diagonalization are then controlled by `max_dim` and `sci_solver_options`. Electron-count overrides (`num_elec_a`, `num_elec_b`), `spin_sq_target`, and `seed` are most useful for reproducibility or debugging. `carryover_threshold` now keeps large-weight determinant strings in the selected-CI pool from one recovery round to the next, while `symmetrize_spin` forces the alpha/beta determinant pools to match in balanced-spin studies. The big levers still remain sample budget and determinant-cap settings.",
      },
      {
        heading: "Typical failure modes",
