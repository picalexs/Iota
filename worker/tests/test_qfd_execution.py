...
from worker.chemistry.algorithms.qfd.execution import prepare_qfd_execution
...
def _prepare(
    *,
    ...
    return prepare_qfd_execution(
        ...
        prepare_dense_spectrum_fn=prepare_spectrum,
    )


def test_prepare_qfd_execution_resolves_dense_resources_and_spectrum() -> None:
    operator = np.eye(8, dtype=complex)
    spectrum = (np.array([0.0]), np.eye(8, 1, dtype=complex), np.array([1.0]))
...
def test_prepare_qfd_execution_resolves_sector_resources_without_dense_matrix() -> None:
    ...
    plan = _prepare(
        ...
        prepare_spectrum=lambda **_: (None, None, None),
    )
...
def test_prepare_qfd_execution_keeps_branch_path_without_local_resources() -> None:
    plan = _prepare(
        branch=True,
        sector=True,
        resolve_operator=lambda _hamiltonian: (_ for _ in ()).throw(
