...
class IBMAdapter(BackendAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self._runtime_failed_submission_ledger: list[dict[str, Any]] = []
    ...
    def create_estimator(self, context: BackendExecutionContext | None = None) -> Any:
        ...
        self._runtime_failed_submission_ledger = []
    ...
    def execution_metadata(self, context: BackendExecutionContext | None = None) -> dict[str, Any]:
        ...
        requested_shots = (
            resolved.requested_shots if resolved.requested_shots is not None else resolved.shots
        )
        configured_precision = (
            resolved.estimator_precision if resolved.estimator_precision > 0.0 else None
        )
        observed_precision = self._last_runtime_observation.get("estimator_precision")
        metadata: dict[str, Any] = {
            ...
            "execution_budget_provenance": {
                "shots": {
                    "requested": requested_shots,
                    "configured": resolved.shots,
                    "provider_observed": (
                        int(observed_shots)
                        if isinstance(observed_shots, (int, float))
                        else None
                    ),
                    "measured": None,
                    "measured_reason": "Runtime metadata does not expose measured shots",
                },
                "estimator_precision": {
                    "requested": (
                        resolved.requested_estimator_precision
                    ...
                    "provider_observed": (
                        float(observed_precision)
                        if isinstance(observed_precision, (int, float))
                        else None
                    ),
                    "measured": None,
                    "derived": _runtime_effective_estimator_precision(resolved),
                    "derived_from_shots": configured_precision is None,
        ...
        if self._transpilation_records:
            metadata["transpilation_records"] = [
                dict(record) for record in self._transpilation_records
            ]
        if self._runtime_submission_ledger:
            metadata["runtime_submission_ledger"] = [
                dict(record) for record in self._runtime_submission_ledger
            ] + [dict(record) for record in self._runtime_failed_submission_ledger]
            metadata["runtime_accounting"] = _runtime_accounting(
                self._runtime_submission_ledger,
                self._runtime_failed_submission_ledger,
            )
        elif self._runtime_failed_submission_ledger:
            metadata["runtime_submission_ledger"] = [
                dict(record) for record in self._runtime_failed_submission_ledger
            ]
            metadata["runtime_accounting"] = _runtime_accounting(
    ...
    def _build_job_observer(
        ...
        def observe(job: Any, metadata: dict[str, Any]) -> Any | None:
            ...
            for record in reversed(self._runtime_submission_ledger):
                if job_id is None or record.get("job_id") == job_id:
                    record["status"] = "provider_observed"
                    record["provider_observed_shots"] = (
                        int(shots) if isinstance(shots, (int, float)) else None
                    )
                    break
    ...
    def _build_estimator_run_transform(
        ...
        def transform(
            ...
            self._record_transpilation_pubs(pubs, transpiler)
        ...
        if not self._last_transpilation_summary:
            for record in records:
                ...
                if isinstance(summary, dict):
                    self._last_transpilation_summary = dict(summary)
                    break
        if not self._last_transpiled_preview:
            preview = transpiler.last_preview
            if preview:
        ...
        return [
            {
                ...
                "metrics": None,
                "unavailable_reason": "PUB was not exposed as a Qiskit circuit",
            }
            for index in range(pub_count)
        ]
...
def _runtime_estimator_options(context: BackendExecutionContext) -> dict[str, Any]:
    options = _runtime_policy_options(context)
    if context.estimator_precision > 0.0:
        options["default_precision"] = context.estimator_precision
    else:
        options["default_shots"] = context.shots
    return options
...
def _runtime_policy_options(context: BackendExecutionContext) -> dict[str, Any]:
    """Return pinned Runtime options for raw or bounded suppression execution."""
    dynamical_decoupling = context.backend_options.get("dynamical_decoupling", False)
    twirling = context.backend_options.get("twirling", False)
    return {
        "resilience_level": _RAW_RUNTIME_POLICY["resilience_level"],
...
def _raw_runtime_policy_metadata(context: BackendExecutionContext) -> dict[str, Any]:
    dynamical_decoupling = context.backend_options.get("dynamical_decoupling", False)
    twirling = context.backend_options.get("twirling", False)
    policy_name = "+".join(
        name
...
def _runtime_accounting(
    ...
    return {
        "submitted_jobs": len(ledger),
        "failed_submissions": len(failures),
        "provider_observed_jobs": sum(
            record.get("status") == "provider_observed" for record in ledger
        ),
        "successful_submissions": sum(
            record.get("status") in {"submitted", "provider_observed"}
            for record in ledger
        ),
        "submitted_pubs": sum(
            record.get("pub_count", 0)
            for record in ledger
            if isinstance(record.get("pub_count"), int)
        ),
        "requested_shots_total": sum(
            (record.get("requested_shots") or 0)
            * (record.get("pub_count") or 0)
            for record in ledger
            if isinstance(record.get("requested_shots"), int)
            and isinstance(record.get("pub_count"), int)
        ),
        "failed_pub_count": sum(
            record.get("pub_count", 0)
...
def _validate_runtime_context(
    ...
    *,
    ...
    for policy_name in ("dynamical_decoupling", "twirling"):
        if not isinstance(context.backend_options.get(policy_name, False), bool):
            raise BackendError(f"IBM Runtime {policy_name} must be a boolean")
...
def _extract_runtime_job_id(job: Any) -> str | None:
...
class _RuntimeTranspiler:
    ...
    def __init__(self, *, backend: Any, context: BackendExecutionContext) -> None:
        ...
        self._circuit_keys: dict[int, str] = {}
        self._circuit_summaries: dict[int, dict[str, Any]] = {}
        self._circuit_previews: dict[int, dict[str, Any]] = {}
    ...
    def transpile_circuit(self, circuit: Any) -> Any:
        ...
        if cached is not None:
            self.last_summary = dict(self._circuit_summaries.get(circuit_id, {}))
            self.last_preview = dict(self._circuit_previews.get(circuit_id, {}))
            return cached
        ...
        self._circuit_keys[circuit_id] = f"circuit-{len(self._circuit_keys) + 1}"
        ...
        self.last_summary.update(
            {
                "transpiler_seed": self._context.backend_options.get("seed_transpiler"),
                "provenance_class": "qss_submitted_isa",
                "provider_final_compilation": "unknown",
            }
        )
        self._circuit_summaries[circuit_id] = dict(self.last_summary)
        self.last_preview = serialize_legacy_circuit_preview(transpiled, style="iqp")
        self._circuit_previews[circuit_id] = dict(self.last_preview)
        return transpiled

    def records_for_pubs(self, pubs: list[Any]) -> list[dict[str, Any]]:
        ...
        for pub_index, pub in enumerate(pubs):
            ...
            if not _is_circuit_like(circuit):
                ...
                continue
            circuit_id = id(circuit)
            summary = self._circuit_summaries.get(circuit_id)
            if summary is None:
                records.append(
                    {
                        "pub_index": pub_index,
                        "circuit_key": self._circuit_keys.get(circuit_id),
                        "provenance_class": "unknown",
                        "provider_final_compilation": "unknown",
                        "metrics": None,
                        "unavailable_reason": "Transpilation summary was not captured",
                    }
                )
                continue
            records.append(
...
def _transpiled_circuit_summary(
    ...
    *,
    ...
    gate_counts = _gate_counts(transpiled)
...
def _gate_counts(circuit: Any) -> dict[str, int | None]:
    ...
    one_qubit = 0
    two_qubit = 0
    excluded = {"barrier", "delay", "measure", "reset"}
    for instruction in instructions:
        ...
        if operation is None and isinstance(instruction, tuple) and instruction:
            ...
            qargs = instruction[1] if len(instruction) > 1 else None
        name = str(getattr(operation, "name", "")).lower()
        if name in excluded:
            continue
        qubit_count = getattr(operation, "num_qubits", None)
        if not isinstance(qubit_count, int) and isinstance(qargs, (list, tuple)):
